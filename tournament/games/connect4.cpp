...
void displayTabP4(const vector<vector<cellP4>> & TabP4){
    ofs << " 0  1  2  3  4  5  6 " << endl;
    for(unsigned i=0; i<6; i++){
        ofs << "----------------------" << endl << "|";
        for(unsigned j=0; j<7; j++){
            if(TabP4[i][j].color == "Empty") ofs << "  " << "|";
            else if(TabP4[i][j].color == "Yellow") ofs << "\033[43m " << "\033[33mJ\033[00m" << "|";
            else ofs << "\033[41m " << "\033[31mR\033[00m" << "|";
        }
        ofs << endl;
    }
    ofs << "----------------------" << endl << " 0  1  2  3  4  5  6 "  << endl;
}
...
int testIfGoodNumber(){
    while(true){
        ...
        else ofs << "le numéro entré n'a pas été reconnu ! Réessayez." << endl;
    }
}
...
cellP4 putRedCoin(vector<vector<cellP4>> & TabP4, const string & t2){
    while(true){
        ...
        ofs << "Le numéro entré n'est pas valide, réessayez !" << endl;
    }
}
...
unsigned p4(const string & t1, const string & t2) { // t1 et t2 correspondent au nom des équipes
    ...
    ofs << "L'équipe " << teams[0] << " est jaune."<< endl;
    ofs << "L'équipe " << teams[1] << " est rouge."<< endl;
    cellP4 cell;
    unsigned cpt = 0;
    ...
    while(true){
        ...
        if (isVictory(TabP4, cell)){
            ...
            displayTabP4(TabP4);
            ofs << "L'équipe jaune " << teams[0] << " a gagné!" << endl;
            break;
        }
        cpt++;
        displayTabP4(TabP4);
        ...
        if (isVictory(TabP4, cell)){
            ...
            displayTabP4(TabP4);
            ofs << "L'équipe rouge " << teams[1] << " a gagné!" << endl;
            break;
        }
        cpt++;
        if (cpt  == 42){
            winner = 2;
            displayTabP4(TabP4);
            ofs << "Aucune équipe n'a gagné, égalité!" << endl;
            break;
        }
    }
