...
void displayChessboard(const vector<piece> & chessboard, const vector<string> & colors, const bool & reversed=false) {
    ...
    bool posColor = true;
    string colName;
    if (colors[reversed]=="blancs") {        // Affichage cote blanc
...
short echecs(const string & t1, const string & t2) {
    ...
    if (turn) colors = {"noirs", "blancs"};
    vector<vector<bool>> castle = {{true, true}, {true, true}};
    ofs << "L'equipe " << teams[turn] << " joue avec les blancs." << endl
         << "L'equipe " << teams[!turn] << " joue avec les noirs." << endl;
    pressEnter();

    ofs << "Pour un gameplay plus agreable dans la console, vous pouvez modifier les couleurs par defaut du terminal pour les faire correspondre aux couleurs classiques d'un echiquier :" << endl
         << "Pieces blanches : #FFF" << endl
         << "Cases blanches  : #E2BD7D" << endl
    ...
    while (true) {
        ...
        displayChessboard(chessboard, colors, turn);
        ofs << endl << "Trait aux " << colors[turn] << endl
        ...
        if (colors[turn]=="noirs") color = 'b', enemyColor='w';
        while (!correctInput) {
